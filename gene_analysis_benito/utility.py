"""Modern Benito dataset loading, normalization, and aggregation utilities."""
...
# ============================================================
# Normalization utilities (DESeq2-style)
# ============================================================

def compute_deseq_size_factors(counts_df):
    """
    Compute DESeq2-style size factors.
    """
    ...
    size_factors = size_factors.replace([0, np.nan, np.inf, -np.inf], np.nan)
...
def normalize_with_size_factors(counts_df):
    """
    Normalize counts using DESeq2 median-ratio method.
    """
    size_factors = compute_deseq_size_factors(counts_df)
    return counts_df.div(size_factors, axis=1)


def transform_data(df, transformed):
    """
    Apply optional transformation to data.
    """
    ...
    if transformed == "sqrt":
        return np.sqrt(df)
    raise ValueError(f"Unknown transformed option: {transformed}")
...
def filter_benito_replicates(
    ...
    if normalize == "deseq":
        df_filtered = normalize_with_size_factors(df_filtered)
    elif normalize in (None, False):
        pass
    else:
        raise ValueError(f"Unknown normalize option: {normalize}")

    df_filtered = transform_data(df_filtered, transformed)
